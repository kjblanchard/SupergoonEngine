...
int LuaCheckFunctionCallParamsAndTypes(lua_State *L, int numArgsOnStack, ...) {
  if (lua_gettop(L) != numArgsOnStack) { // Lua stack is 1-based
    sgLogWarn("Bad number of arguments passed to C function from Lua: expected "
              "%d, got %d",
...
rfalse:
  sgLogWarn("Bad parameter passed into Lua function at stack position %d, "
...
// 		sgLogWarn("Bad number of arguments passed to C function from
// lua"); 		return false;
// 	}
// 	va_list argP;
...
// 		++luaPos;
// 		if (!param) {
// 			sgLogWarn("Could not get arg from vaargs, returning
// false"); 			return false;
// 		}
// 		switch (param) {
// 			case LuaFunctionParameterTypeInt:
...
// rfalse:
// 	va_end(argP);
// 	sgLogWarn("Bad parameter passed into lua function, expecting %s at pos
// %d but got %s", LuaGetParamType(param), luaPos, LuaGetTypeStringi(L,
// luaPos)); 	return false;
// }
