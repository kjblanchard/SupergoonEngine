...
static long loadBufferData(Stream* stream, BufferFillFlags* buff_flags) {
	...
	*buff_flags = 0;
	...
	while (total_buffer_bytes_read < BGM_BUFFER_SAMPLES) {
		...
		request_size = (total_buffer_bytes_read + request_size + stream->TotalBytesReadThisLoop <= stream->BgmData->LoopEnd)
						   ? request_size
						   : stream->BgmData->LoopEnd - (total_buffer_bytes_read + stream->TotalBytesReadThisLoop);
		if (request_size == 0) {
		...
		int current_pass_bytes_read = ov_read(stream->BgmData->VorbisFile, (char*)stream->Buffer + total_buffer_bytes_read, request_size, 0, sizeof(short), 1, 0);
