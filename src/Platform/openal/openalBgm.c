...
static void getLoopPointsFromVorbisComments(Bgm* bgm, double* loopBegin, double* loopEnd) {
	...
	if (!vc) {
		sgLogWarn("Error retrieving vorbis comments for , setting to 0");
		*loopBegin = *loopEnd = 0;
		return;
	}
...
void BgmLoad(Bgm* bgm) {
	...
	if (result != 0) {
		sgLogError("Could not open audio in %s: %d", bgm->Filename, result);
		return;
	}
	...
	if (!bgm->Format) {
		sgLogError("Unsupported channel count: %d", bgm->VorbisInfo->channels);
