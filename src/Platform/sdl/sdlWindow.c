...
void CreateWindowImpl(void) {
	...
	sgLogDebug("[WIN] Set orientation hint: LandscapeLeft LandscapeRight");
#endif

	sgLogDebug("[WIN] Creating window: %s %dx%d flags=0x%X", name, _windowWidth, _windowHeight, flags);
	...
	sgLogDebug("[WIN] Window created: handle=%p", (void*)_window.Handle);
	SDL_GetWindowSizeInPixels(_window.Handle, &_windowWidth, &_windowHeight);
	sgLogDebug("[WIN] Window pixel size: %d x %d", _windowWidth, _windowHeight);
	onWindowResize();
}
