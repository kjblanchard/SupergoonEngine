...
extern void DrawTextureImpl(Texture* texture, Shader* shader, RectangleF* dst, RectangleF* src, bool camera, float scale, bool flipY, Color* color);
...
void TextureLoadFromPngBuffer(Texture* texture, const char* filepath, char* buf, size_t sz) { TextureLoadFromPngBufferImpl(texture, filepath, buf, sz); }
